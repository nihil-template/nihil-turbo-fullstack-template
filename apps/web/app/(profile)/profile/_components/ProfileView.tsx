        ...
        <CardContent className='space-y-4'>
          <div className='grid grid-cols-1 md:grid-cols-2 gap-4'>
            <div>
              ...
              <p className='text-lg font-semibold'>{session.userNm || '미설정'}</p>
            </div>
            <div>
              <p className='text-sm font-medium text-muted-foreground'>이메일</p>
              <p className='text-lg font-semibold'>{session.emlAddr}</p>
            </div>
            <div>
              <p className='text-sm font-medium text-muted-foreground'>역할</p>
              <p className='text-lg font-semibold'>{session.userRole || '사용자'}</p>
            </div>
            <div>
              <p className='text-sm font-medium text-muted-foreground'>가입일</p>
              <p className='text-lg font-semibold'>
                {session.crtDt
                  ? new Date(session.crtDt).toLocaleDateString('ko-KR')
                  : '알 수 없음'}
              </p>
            </div>
          </div>
